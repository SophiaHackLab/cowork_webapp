...
const desks = [
    ...
    { id: 7, x: 125, y: 440, width: 125, height: 60, available: false, deskDirection: 'bottom' },
    { id: 8, x: 250, y: 440, width: 125, height: 60, available: false, deskDirection: 'bottom' },
];
...
onMounted(() => {
    ...
    const handleHover = (desk: any, rect: Rect, chair: Rect, circle?: Circle) => {
        const mouseover = () => {
        ...
        (circle ?? rect).on('mouseover', () => mouseover())
            .on('mouseout', () => mouseout());
        chair.on('mouseover', () => mouseover())
            .on('mouseout', () => mouseout());
    };
    desks.forEach(desk => {
        ...
        if (desk.deskDirection === 'top')
            chair = draw.rect(40, 20).attr({ fill: desk.available ? '#272727' : '#17191c', stroke: '#09090b', 'stroke-width': 2, rx: 5, rt: 0 }).move(desk.x + desk.width / 2 - 20, desk.y - 20);
        else
            chair = draw.rect(40, 20).attr({ fill: desk.available ? '#272727' : '#17191c', stroke: '#09090b', 'stroke-width': 2, rx: 5, rb: 0 }).move(desk.x + desk.width / 2 - 20, desk.y + desk.height);
        chair.css({ cursor: desk.available ? 'pointer' : 'not-allowed' })


        const rect = draw.rect(desk.width, desk.height)
            .attr({ fill: desk.available ? '#272727' : '#17191c', stroke: '#09090b', 'stroke-width': 4, rx: 10 })
        ...
        let circle: Circle
        if (desk.available)
            circle = draw.circle(10).attr({ fill: '#6cd244' }).move(desk.x + desk.width / 2 - 5, desk.y + desk.height / 2 - 5)
        else
            circle = draw.circle(10).attr({ fill: 'red' }).move(desk.x + desk.width / 2 - 5, desk.y + desk.height / 2 - 5)

        circle.css({ cursor: desk.available ? 'pointer' : 'not-allowed' })
    ...
    // Center the SVG dynamically
    // const resizeObserver = new ResizeObserver(() => {
    //     const containerRect = floorplanContainer.value.getBoundingClientRect();
    //     const viewBox = draw.viewbox();
    //     const centerX = (viewBox.width - width / panZoom.getZoom()) / 2;
    //     const centerY = (viewBox.height - height / panZoom.getZoom()) / 2;
    //     panZoom.pan({ x: -centerX, y: -centerY });
    // });

    // resizeObserver.observe(floorplanContainer.value);
});
